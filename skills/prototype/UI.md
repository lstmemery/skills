...
Record which variant won and why, or what feedback remains needed. Apply the following cleanup only when [SKILL.md](SKILL.md) selects capture for this request and a winner has been chosen:

- **Sub-shape A**: fold the winner into the existing page; drop the losing variants and the switcher from main.
- **Sub-shape B**: promote the winning variant to a real route; drop the throwaway route and the switcher from main.

For that capture branch, preserve the full variant set on the throwaway branch as the primary source. Keep only the validated production decision in main; the entrypoint owns the capture condition.
